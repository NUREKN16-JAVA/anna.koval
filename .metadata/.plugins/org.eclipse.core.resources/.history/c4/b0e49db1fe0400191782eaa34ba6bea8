...
public class EditPanel extends AddPanel {
	...
	private JPanel getFieldPanel() {
		if (fieldPanel == null) {
			...
			addLabeledField(fieldPanel, ua.nure.kn.koval.usermanagement.util.Messages.getString("EditPanel.firstName"), //$NON-NLS-1$
					getFirstNameField());
			addLabeledField(fieldPanel, ua.nure.kn.koval.usermanagement.util.Messages.getString("EditPanel.lastName"), //$NON-NLS-1$
					getLastNameField());
			addLabeledField(fieldPanel,
					ua.nure.kn.koval.usermanagement.util.Messages.getString("EditPanel.dateOfBirth"), //$NON-NLS-1$
					getDateOfBirthField());
		}
		return fieldPanel;
	}
	...
	private JButton getOkButton() {
		if (okButton == null) {
			...
			okButton.setText(ua.nure.kn.koval.usermanagement.util.Messages.getString("EditPanel.saveButton")); //$NON-NLS-1$
	...
	private JButton getCancelButton() {
		if (cancelButton == null) {
			...
			cancelButton.setText(ua.nure.kn.koval.usermanagement.util.Messages.getString("EditPanel.cancelButton")); //$NON-NLS-1$
	...
	public void actionPerformed(ActionEvent e) {
		if ("ok".equalsIgnoreCase(e.getActionCommand())) { //$NON-NLS-1$
			...
			try {
				...
				getDateOfBirthField().setBackground(Color.PINK);
				return;
			}
			try {
